...
class InviteMembersForm < ComplexForms
  ...
  def send_invite(membership)
    # Modified line to handle both object and string scenarios
    invited_by = @current_user.person.is_a?(Person) ? @current_user.person : @current_user.person.name
    Invitation.new(membership: membership,
                   invited_by: invited_by).send_invite
    ...
    @memberships.each do |membership|
      ...
      if membership.attendance == 'Not Yet Invited'
        ...
        if invite.blank?
          ...
          # Modified line to handle both object and string scenarios
          invite.invited_by = @current_user.person.is_a?(Person) ? @current_user.person : @current_user.person.name

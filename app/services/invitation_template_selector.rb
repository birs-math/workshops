...
class InvitationTemplateSelector
  ...
  def template_name
    base_template = membership.role.match?('Virtual') ? virtual_template : normal_template
    
    # Check if the template file exists
    return base_template if File.exist?("#{template_path}/#{base_template}.text.erb")
    
    # If template doesn't exist, try simplified versions
    event = membership.event
    format = event.event_format
    attendance = membership.attendance
    
    # Try with just "Workshop" if event_type contains "Workshop"
    if event.event_type.include?('Workshop')
      simplified = "#{format}-Workshop-#{attendance}"
      return simplified if File.exist?("#{template_path}/#{simplified}.text.erb")
    end
    
    # If we have a 5 Day Workshop template, use that
    five_day = "#{format}-5 Day Workshop-#{attendance}"
    return five_day if File.exist?("#{template_path}/#{five_day}.text.erb")
    
    # If we have a 2 Day Workshop template, use that
    two_day = "#{format}-2 Day Workshop-#{attendance}"
    return two_day if File.exist?("#{template_path}/#{two_day}.text.erb")
    
    # Return the original template name (will likely fail, but the error will be clear)
    base_template
  end
  def template_path
